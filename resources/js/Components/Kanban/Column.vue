...
const updateCardOrder = (event) => {
    const targetColumn = props.column;
    const orderPayload = props.column.cards.map((card, index) => ({
        id: card.id,
        order: index + 1,
        column_id: targetColumn.id
    }));

    router.post(route('cards.reorder'), {
        cards: orderPayload
    }, {
        preserveState: true,
        preserveScroll: true
...
<template>
    <div class="bg-gray-100 rounded-lg shadow-sm w-80 flex-shrink-0 flex flex-col">
        <div class="p-4 border-b border-gray-200 flex justify-between items-center">
            ...
            <h2 v-if="!showEditColumnForm" class="text-lg font-bold text-gray-700">{{ column.name }}
            </h2>
            <form v-else @submit.prevent="updateColumn" class="flex-grow">
                <input type="text" v-model="editColumnForm.name"
                    class="w-full p-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </form>
            <div>
                <EllipsisHorizontalIcon class="h-4 w-4 text-gray-600 cursor-grab handle" />
                <EllipsisHorizontalIcon class="h-4 w-4 text-gray-600 -mt-3 cursor-grab handle" />
            </div>
            <div v-if="role !== 'viewer'" class="flex space-x-2 ml-2">
                <button v-if="!showEditColumnForm" @click="showEditColumnForm = true"
                ...
                <button @click="deleteColumn" class="text-red-500 hover:text-red-700">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd"
                            d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 011-1h4a1 1 0 110 2H8a1 1 0 01-1-1zm-1 3a1 1 0 000 2h8a1 1 0 100-2H6z"
                            clip-rule="evenodd" />
                    </svg>
                </button>
            </div>
        ...
        <div class="p-4">
            <div class="space-y-4 overflow-y-auto max-h-96 scrollbar-hide">
                <draggable v-model="column.cards" group="cards" item-key="id" handle=".card-handle"
                    @change="updateCardOrder" class="space-y-4 overflow-y-auto max-h-96 scrollbar-hide">
                    <template #item="{ element }">
                        <Card :card="element" :role="role" />
                    </template>
                </draggable>
            </div>
        </div>
        <div v-if="role !== 'viewer'" class="p-4">
            <form v-if="showNewCardForm" @submit.prevent="addCard">
                <input type="text" v-model="form.title" placeholder="Enter card title"
                ...
                <div class="flex space-x-2">
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Add
                        Card</button>
                    <button type="button" @click="showNewCardForm = false"
                        class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">Cancel</button>
                </div>
            </form>
            <button v-else @click="showNewCardForm = true"
                class="w-full flex items-center justify-center text-sm font-medium text-gray-600 hover:text-gray-800">
